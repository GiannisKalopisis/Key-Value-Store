...
public class TestTrieStructure {
    ...
    public static void main(String[] args) {
        ...
        if (testNode != null) {
            System.out.println(testNode.getLeafData());
        }
        else {
            System.out.println("Word key3 doesn't exists in trie");
        }

        String[] splittedData = parser.parseKeyValue("key1 -> [ ]");
        trie.insert(splittedData[0], splittedData[1].split("-> \\[", 2)[1]);
        splittedData = parser.parseKeyValue("key2 -> [ eye -> \"QFwh\" ]]");
        trie.insert(splittedData[0], splittedData[1].split("-> \\[", 2)[1]);
        splittedData = parser.parseKeyValue("key3 -> [ card -> 70 | level -> 56 | patient -> 70 | strategy -> [ address -> \"LfNw\" | height -> 56 ] | small -> 13.124782 ]");
        trie.insert(splittedData[0], splittedData[1].split("-> \\[", 2)[1]);
        splittedData = parser.parseKeyValue("key4 -> [ center -> 28.491962 | image -> [ bank -> \"nrbp\" | memory -> 3 ] ]");
        trie.insert(splittedData[0], splittedData[1].split("-> \\[", 2)[1]);
